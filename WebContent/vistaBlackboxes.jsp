...
<table class="table">
	...
	<tbody>
		...
		<tr>
			...
			<td class="text-center"><a href="#" onclick="editarBlackbox('<%= blackbox.getIdentificador() %>')"><i class="fas fa-pen-square"></i></a></td>
...
<script>
	function crearBlackbox() {
		$.get("AddBlackbox", function(htmlExterno) {$("#contenido").html(htmlExterno)});
	}
	
	function editarBlackbox(uid) {
		$.get("EditarBlackbox", {uid: uid}, function(htmlExterno) {$("#contenido").html(htmlExterno)});
	}
